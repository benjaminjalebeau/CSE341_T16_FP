...
const getAllCourses = async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({ message: 'Error retrieving courses', error: err.message });
    }
};
...
const getCourseById = async (req, res) => {
    ...
    try {
        ...
        const result = await mongodb.getDatabase().db().collection('courses').find({ _id: courseId }).toArray();
        ...
        res.status(400).json({ message: 'Error retrieving course', error: err.message });
    }
};
...
const getCoursesByTeacherId = async (req, res) => {
    ...
    try {
        ...
        const result = await mongodb.getDatabase().db().collection('courses').find({ teacherId: teacherId }).toArray();
        ...
        res.status(400).json({ message: 'Error retrieving courses by teacher ID', error: err.message });
    }
};
...
const getCoursesByStudent = async (req, res) => {
    ...
    try {
        ...
        const result = await mongodb.getDatabase().db().collection('courses').find({ studentsEnrolled: studentId }).toArray();
        ...
        res.status(400).json({ message: 'Error retrieving courses for student', error: err.message });
    }
};
...
const updateCourse = async (req, res) => {
    ...
    try {
        const response = await mongodb.getDatabase().db().collection('courses').updateOne(
            { _id: courseId },
            { $set: updatedCourse }
        );
...
const deleteCourse = async (req, res) => {
    ...
    try {
        const response = await mongodb.getDatabase().db().collection('courses').deleteOne({ _id: new ObjectId(courseId) });
...
module.exports = { getAllCourses, getCourseById, getCoursesByTeacherId, getCoursesByStudent, createCourse, updateCourse, deleteCourse };